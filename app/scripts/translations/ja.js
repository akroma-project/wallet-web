...
ONBOARD_secure_1_content__3     :  'いつも、必ず、ＵＲＬ：「https://wallet.akroma.org」を確認してください。', /*Always check the URL: `https://wallet.akroma.org`.*/
...
ERROR_28                    : '(error_29) お財布を操作するためには**Keystore/JSONとパスワードか秘密鍵が必要** 保存してから、外部バックアップしてください！ここで保存しないとお財布が使用できなくなります。詳細はヘルプページを参照してください。(https://wallet.akroma.org/#help) (You need this `Keystore File + Password` or the `Private Key` (next page) to access this wallet in the future. ) ',
...
TXFEE_Desc                  : 'この処理料金は自分の処理をブロックに配置するためにマイナーに支払われます。「ガスリミット」＊「ガス価格」で算出されます。 [GWEI -> ETHの単位変換はこちらです。](https://wallet.akroma.org/helpers.html)',　/*The TX Fee is paid to miners for including your TX in a block. It is the `gas limit` * `gas price`. [You can convert GWEI -> ETH here](https://wallet.akroma.org/helpers.html)*/
...
HELP_Sec_Desc_4             : 'Check the URL -- This site is being served through GitHub and you can see the source code here: [https://github.com/kvhnuke/etherwallet/tree/gh-pages](https://github.com/kvhnuke/etherwallet/tree/gh-pages) to [https://wallet.akroma.org](https://wallet.akroma.org). ',
